...
- **Automated tests (lifecycle/cleanup/regression):** PASS (Всего 27 unit тестов: 15 тестов в `test_storage_security.py` [11 восстановленных оригинальных тестов безопасности из `009bb799` + 4 добавленных теста для `ensure_readonly_root_dir`], 7 тестов в `test_macos_startup_ux.py`, 2 теста в `test_frozen_paths.py`, 3 теста в `test_macos_lifecycle.py`. Все 27 unit тестов прошли успешно. `packaged_smoke_test.py` завершился успешно с кодом 0, подтверждая HTTP 200 на `/`, `/static/index.html`, валидный JSON от `/api/preflight`, изолированный `OUT_DIR` вне bundle и чистый выход по SIGINT).
